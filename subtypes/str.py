...
class FuzzyMatcher:
    def __init__(self, tokenize: bool = False, partial: bool = False) -> None:
        self.tokenize = self.partial = None  # type: bool
        self.configure(tokenize=tokenize, partial=partial)
    ...
    def configure(self, tokenize: bool = False, partial: bool = False) -> FuzzyMatcher:
...
class Str(collections.UserString, str):  # type: ignore
    regex, re, fuzzy = regexmod, RegexSettings(), FuzzyMatcher()
    ...
    def find_all(self, substring: str, regex: bool = False, overlapping: bool = True, not_within: List[Tuple[str, str]] = None) -> List[int]:
        ...
        return [match.start() for match in regexmod.finditer(fr"{prefix}({pattern})", self.data, overlapped=overlapping if not_within is None else False, flags=self.re() if regex else 0) if match.group(1)]
